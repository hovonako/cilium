...
func New(vp *viper.Viper) *cobra.Command {
	...
	vp.BindPFlag(keyDebug, cmd.Flags().Lookup(keyDebug))

	cmd.Flags().Bool(
		keyPprof, false, "Enable serving the pprof debugging API",
	)
	vp.BindPFlag(keyPprof, cmd.Flags().Lookup(keyPprof))

	cmd.Flags().Bool(
		keyGops, true, "Run gops agent",
	)
	vp.BindPFlag(keyGops, cmd.Flags().Lookup(keyGops))

	cmd.Flags().Duration(
		keyDialTimeout,
		defaults.DialTimeout,
		"Dial timeout when connecting to hubble peers")
	vp.BindPFlag(keyDialTimeout, cmd.Flags().Lookup(keyDialTimeout))

	cmd.Flags().Duration(
		keyRetryTimeout,
		defaults.RetryTimeout,
		"Time to wait before attempting to reconnect to a hubble peer when the connection is lost")
	vp.BindPFlag(keyRetryTimeout, cmd.Flags().Lookup(keyRetryTimeout))

	cmd.Flags().String(
		keyListenAddress,
		defaults.ListenAddress,
		"Address on which to listen")
	vp.BindPFlag(keyListenAddress, cmd.Flags().Lookup(keyListenAddress))

	cmd.Flags().String(
		keyPeerService,
		defaults.HubbleTarget,
		"Address of the server that implements the peer gRPC service")
	vp.BindPFlag(keyPeerService, cmd.Flags().Lookup(keyPeerService))
	...
	vp.BindPFlag(keySortBufferMaxLen, cmd.Flags().Lookup(keySortBufferMaxLen))
	...
	vp.BindPFlag(keySortBufferDrainTimeout, cmd.Flags().Lookup(keySortBufferDrainTimeout))
